...
import { Zap, ArrowLeft, Eye, EyeOff, Wind, Moon, Sun } from 'lucide-react';
...
export default function AuthUser() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isSignup) {
        const res = await axios.post('http://localhost:5000/auth/signup', { ...form, role: 'user' });
        login(res.data.token, res.data.user);
      } else {
        const res = await axios.post('http://localhost:5000/auth/login', { email: form.email, password: form.password, role: 'user' });
        login(res.data.token, res.data.user);
      }
      navigate('/dashboard');
    } catch (err) {
      ...
      <div className="flex-1 flex items-center justify-center px-6 py-12">
        <div className="w-full max-w-md bg-white dark:bg-slate-900/80 border border-slate-200 dark:border-slate-800 rounded-2xl p-8 shadow-xl dark:shadow-2xl">
          <h2 className="text-2xl font-bold text-slate-800 dark:text-white text-center mb-2">
            {isSignup ? 'Energy User Signup' : 'Energy User Login'}
          </h2>
          <p className="text-slate-500 dark:text-slate-400 text-sm text-center mb-8">
            {isSignup ? 'Create an account to receive and manage energy tokens' : 'Login to view your energy stockings'}
          </p>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <>
                <div>
                  ...
                  <input name="name" value={form.name} onChange={handleChange} required
                    className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700 rounded-xl text-slate-900 dark:text-white placeholder:text-slate-400 dark:placeholder:text-slate-500 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none transition-all"
            ...
            <div>
              ...
              <div className="relative">
                <input name="password" type={showPass ? 'text' : 'password'} value={form.password} onChange={handleChange} required
                  className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700 rounded-xl text-slate-900 dark:text-white placeholder:text-slate-400 dark:placeholder:text-slate-500 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none transition-all pr-12" />
            ...
            <button type="submit" disabled={loading}
              className="w-full py-3.5 bg-gradient-to-r from-blue-600 to-blue-500 hover:from-blue-500 hover:to-blue-400 text-white font-semibold rounded-xl transition-all shadow-lg shadow-blue-900/30 disabled:opacity-50"
            >
          ...
          <p className="text-center text-sm text-slate-500 dark:text-slate-400 mt-6">
            {isSignup ? "Already have an account? " : "Don't have an energy user account? "}
            <button onClick={() => { setIsSignup(!isSignup); setError(''); }} className="text-blue-600 dark:text-blue-400 hover:text-blue-500 dark:hover:text-blue-300 font-semibold">
